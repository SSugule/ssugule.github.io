...
export default function App() {
  ...
  const handleAddTagToPost = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isSignedUp) {
      alert('Ошибка: Редактирование тегов доступно только авторизованным пользователям.');
      return;
    }
  ...
  const handleOpenEditPost = () => {
    if (!isSignedUp) {
      alert('Ошибка: Редактирование публикаций доступно только авторизованным пользователям.');
      return;
    }
    if (!selectedPost) return;
  ...
  const handleUploadEditFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!isSignedUp) {
      alert('Ошибка: Загрузка файлов доступна только авторизованным пользователям.');
      return;
    }
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  const handleDeletePost = async () => {
    if (!isSignedUp) {
      alert('Ошибка: Удаление публикаций доступно только авторизованным пользователям.');
      return;
    }
    if (!selectedPost) return;
    try {
  ...
  const handlePublishPost = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isSignedUp) {
      alert('Ошибка: Публикация доступна только авторизованным пользователям.');
      return;
    }
    if (!uploadUrl.trim()) {
                  ...
                  <div className="space-y-1 text-zinc-400">
                    ...
                    {isSignedUp && (
                      <div className="pt-2.5 border-t border-zinc-900">
                        {showDeleteConfirm ? (
                          <div className="bg-red-955 border border-red-900/30 p-2 rounded space-y-2 mt-1">
                          ...
                          <button
                            onClick={() => setShowDeleteConfirm(true)}
                            className="w-full py-1.5 bg-red-95/20 hover:bg-red-90/30 border border-red-95/50 hover:border-red-500/60 text-rose-455 hover:text-rose-350 text-[10px] font-bold rounded flex items-center justify-center gap-1.5 transition cursor-pointer font-mono uppercase"
                                    ...
                                    .map(tag => (
                                      <li key={tag} className="group/tag flex items-center justify-between text-[11px] font-mono py-0.5 hover:bg-zinc-900/40 px-1 rounded">
                                        ...
                                        <div className="flex items-center gap-1">
                                          <button 
                                            onClick={() => { handleQueryAddTag(tag); setSelectedPost(null); }}
                                            className="px-1 text-[9px] font-sans font-bold text-emerald-400 hover:bg-emerald-950/40 border border-emerald-955/30 rounded cursor-pointer transition select-none"
                                            title="Добавить"
                                          >+</button>
                                          <button 
                                            onClick={() => { handleQueryExcludeTag(tag); setSelectedPost(null); }}
